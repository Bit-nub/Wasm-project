...
RUN apt-get update -y > /dev/null 2>&1 && \
    ...
    apt-get install -y --fix-missing build-essential llvm-14 lld-14 clang-14 > /dev/null 2>&1 && \
    echo "---- Installation of build-essential llvm-14 lld-14 clang-14 is complete !" && \
